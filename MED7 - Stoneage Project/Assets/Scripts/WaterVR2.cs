...
namespace UnityStandardAssets.Water
{
    ...
    public class WaterVR2 : MonoBehaviour
    {
        ...
        private RenderTexture m_ReflectionTextureLeftEye;
        private RenderTexture m_ReflectionTextureRightEye;
        ...
        private Texture2D leftEyeTex = null;
        private Texture2D rightEyeTex = null;


        private Texture2D getLeftEyeTexture()
        {

            if (leftEyeTex == null)
            {
                leftEyeTex = new Texture2D(textureSize, textureSize);
                ...
                for (int y = 0; y < textureSize; y++)
                {
                    for (int x = 0; x < textureSize; x++)
                    {
                        ...
                        leftEyeTex.SetPixel(x, y, Color.blue);
        ...
        private Texture2D getRightEyeTexture()
        {

            if (rightEyeTex == null)
            {
                rightEyeTex = new Texture2D(textureSize, textureSize);
                ...
                for (int y = 0; y < textureSize; y++)
                {
                    for (int x = 0; x < textureSize; x++)
                    {
                        ...
                        rightEyeTex.SetPixel(x, y, Color.red);
        ...
        public void OnWillRenderObject()
        {
            ...
            if (mode >= WaterMode.Reflective)
            {
                if (cam.stereoEnabled)
                {
                    ...
                    {
                        // RIGHT EYE!
                        Vector3 eyePos = cam.transform.TransformPoint(new Vector3(5.0f * cam.stereoSeparation, 0, 0));
        ...
        void RenderReflection(Camera reflectionCamera, RenderTexture targetTexture, Vector3 camPos, Quaternion camRot, Matrix4x4 camProjMatrix)
        {
            ...
            reflectionCamera.cullingMask = ~(1 << 9) & reflectLayers.value;
        ...
        void OnDisable()
        {
            if (m_ReflectionTextureLeftEye)
            {
                DestroyImmediate(m_ReflectionTextureLeftEye);
                m_ReflectionTextureLeftEye = null;
            }
            if (m_ReflectionTextureRightEye)
            {
                DestroyImmediate(m_ReflectionTextureRightEye);
                m_ReflectionTextureRightEye = null;
            }
        ...
        void CreateWaterObjects(Camera currentCamera, out Camera reflectionCamera, out Camera refractionCamera)
        {
            ...
            if (mode >= WaterMode.Reflective)
            {
                ...
                if (!m_ReflectionTextureLeftEye || m_OldReflectionTextureSize != textureSize)
                {
                    if (m_ReflectionTextureLeftEye)
                    {
                        DestroyImmediate(m_ReflectionTextureLeftEye);
                    }
                    m_ReflectionTextureLeftEye = new RenderTexture(textureSize, textureSize, 16);
                    m_ReflectionTextureLeftEye.name = "__WaterReflection" + GetInstanceID();
                    m_ReflectionTextureLeftEye.isPowerOfTwo = true;
                    m_ReflectionTextureLeftEye.hideFlags = HideFlags.DontSave;
                }
                if (!m_ReflectionTextureRightEye || m_OldReflectionTextureSize != textureSize)
                {
                    if (m_ReflectionTextureRightEye)
                    {
                        DestroyImmediate(m_ReflectionTextureRightEye);
                    }
                    m_ReflectionTextureRightEye = new RenderTexture(textureSize, textureSize, 16);
                    m_ReflectionTextureRightEye.name = "__WaterReflection1" + GetInstanceID();
                    m_ReflectionTextureRightEye.isPowerOfTwo = true;
                    m_ReflectionTextureRightEye.hideFlags = HideFlags.DontSave;
                }
